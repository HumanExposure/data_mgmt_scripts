...
i=0
template = csv.reader(open('Factotum_Univar_SDS_unextracted_documents_20250819.csv')) 
for row in template:
    filename=row[1]
    ID = row[0]
    ...
    if 'quakercolor' not in cleaned.replace(' ',''): 
        continue
    # i+=1
    # print(i, ID, filename)
    ...
    prodname = prodname.split('other means')[0].split('product code')[0]
    ...
    date=date.split('\n')[0].split('version')[0].split('supersedes')[0].strip(':. ')
    date = re.sub(' +', ' ', date)
    ...
    cat = cat.split('1.3')[0].split('restrictions on use')[0].split('uses advised against')[0]
    cat = (cat).replace('\n',' ').strip(':.- ')
    cat = re.sub(' +', ' ', cat)
    ...
    for line in lines:
        ...
        if betweenPages == True: 
            ...
            if 'revision date' in line or 'rules and regulations' in line:
                betweenPages=False
            continue
        if 'page ' in line or 'revision date' in line:
            betweenPages=True
            # print(ID,filename,line)
            continue
        ...
        if any(x in line for x in ['________','3.1. substances','3.2. mixtures','chemical name cas','*the exact percentage','synonyms','chemical characterization','the product contains','withheld as a']):     
...
df.to_csv('Univar Quaker Color extracted text.csv',index=False, header=True)
