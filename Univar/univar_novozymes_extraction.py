...
i=0
template = csv.reader(open('Factotum_Univar_SDS_unextracted_documents_20250829.csv')) 
for row in template:
    filename=row[1]
    ID = row[0]
    ...
    if 'novozymes' not in cleaned.replace(' ',''): 
        continue
    ...
    prodname = prodname.split('chemical name')[0]
    ...
    rev=rev.split('\n')[0].split('compliant')[0].split('/')[0].strip(':. ')
    rev = re.sub(' +', ' ', rev)
        
        
    #Raw category
    if 'use of the substance/preparation' in cleaned:
        cat = cleaned.split('use of the substance/preparation')[1]
    elif 'relevant identified uses of the substance or mixture and uses advised against' in cleaned:
    ...
    cat = cat.split('supplier')[0].split('1.3 details')[0].split('contact manufacturer')[0]
    cat = (cat).replace('\n',' ').strip(':.- ')
    cat = re.sub(' +', ' ', cat)
    ...
    sec3=sec3.split('section 4')[0].split('4. first')[0]
    lines = sec3.split('\n')
    ...
    for line in lines:
        ...
        if betweenPages == True: 
            ...
            if '______________' in line or 'version no' in line:
                betweenPages=False
            continue
        ...
        if any(x in line for x in ['______________','chemical name cas','revision date:','the exact percentage','contributes to the ghs','cas no iub','chemicalname','the product contains no substances','ontributing to the classification']):     
            continue #skip these lines
        
        
        
        if line in ['rethink tomorrow','hazardous components']: 
            continue
...
df.to_csv('Univar novozymes Chemical extracted text.csv',index=False, header=True)
