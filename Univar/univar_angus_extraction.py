...
i=0
template = csv.reader(open('Factotum_Univar_SDS_unextracted_documents_20250828.csv')) 
for row in template:
    filename=row[1]
    ID = row[0]
    ...
    if 'anguschemical' not in cleaned.replace(' ',''): 
        continue
    ...
    prodname = prodname.split('manufacturer')[0].split('issue date')[0]
    ...
    cat = cat.split('the angus chemical company')[0].split('2. hazards')[0]
    cat = (cat).replace('recommended use','').replace('\n',' ').strip(':.- ')
    cat = re.sub(' +', ' ', cat)
    ...
    for line in lines:
        ...
        if any(x in line for x in ['trademark of angus chemical company','issue date','this product is','chemical name cas','no hazardous ingredients','substance / mixture']):     
...
df.to_csv('Univar Angus Chemical extracted text.csv',index=False, header=True)
