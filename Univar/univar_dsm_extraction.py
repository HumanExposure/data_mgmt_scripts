...
def cleanLine(line):
    ...
    line=line.replace('é','e').replace('É','e').replace('À','a').replace('≤','<=').replace('≥','>=').replace('','').replace('','')#.replace('α','alpha').replace('ω','omega').replace('β','beta').replace('γ','gamma')
...
i=0
template = csv.reader(open('Factotum_Univar_SDS_unextracted_documents_20250818.csv')) 
for row in template:
    filename=row[1]
    ID = row[0]
    ...
    if 'dsm.com' not in cleaned.replace(' ',''): 
        continue
    # i+=1
    # print(i, ID, filename)
    ...
    prodname = prodname.split('substance name')[0].split('manufacturer')[0].split('1.2 relevant')[0]
    ...
    date=date.split('\n')[0].split('date of last issue')[0].split('print date')[0].strip(':. ')
    date = re.sub(' +', ' ', date)
    ...
    rev=rev.split('revision date')[0].strip(':. ')
    rev = re.sub(' +', ' ', rev)
    ...
    elif 'use of the :' in cleaned: 
        cat = cleaned.split('use of the :')[1]
    else:
        print('raw category',ID,filename)
    cat = cat.split('1.3')[0].split('section 2')[0]
    cat=cat.replace('substance/mixture','').replace('recommended use','')
    cat = (cat).replace('\n',' ').strip(':.- ')
    cat = re.sub(' +', ' ', cat)
    ...
    for line in lines:
        ...
        if any(x in line for x in ['synonyms :','substance name :','brief description of the','molecular formula :','cas-no.','concentration (% w/w)','actual concentration is withheld','component cas']):     
            continue #skip these lines
        
        if line in ['hazardous components','no hazardous ingredients','further ingredients']: 
            continue #skip these lines
...
df.to_csv('Univar DSM extracted text.csv',index=False, header=True)
