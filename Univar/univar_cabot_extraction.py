...
def cleanLine(line):
    ...
    line=line.replace('é','e').replace('É','e').replace('À','a').replace('≤','<=').replace('≥','>=').replace('','').replace('','').replace('α','alpha').replace('ω','omega').replace('β','beta').replace('γ','gamma').replace('Ò','')
...
i=0
template = csv.reader(open('Factotum_Univar_SDS_unextracted_documents_20260121.csv')) 
for row in template:
    filename=row[1]
    ID = row[0]
    ...
    if 'cabot' not in cleaned.replace(' ',''): 
        continue
    ...
    prodname = prodname.split('other means of')[0].split('product code')[0]
    ...
    date=date.split('\n')[0].split('revision number')[0].strip(':. ')
    date = re.sub(' +', ' ', date)
    ...
    cat = cat.split('restrictions on use')[0]
    ...
    #     # print(ID,filename,line)
        ...
        if any(x in line for x in ['synonyms','chemical name','product code','page ','_________','the hyphen (-) means','regulatory information is found under','additional information','regulatory information','registry #','hmira','information review act','hs code']):     
            # print(ID,filename,line)
            continue #skip these lines

        # print(ID,filename,line)
        
        if line in ['other information:','substance']: 
            continue #skip these lines
    ...
    for c in range(0,len(chem)): 
        ...
        minC.append('')
        maxC.append('')
        centC[c]=centC[c].strip('- ')
        centC[c]=centC[c].replace('%','').replace('/','').replace('*','').replace('|','').strip()
...
df.to_csv('Univar Cabot extracted text.csv',index=False, header=True)
