...
# DUTCH I - PLASTICS
# Commented out because I editted the CSV file after running the code and don't want the code to undo what I did.
# i = 18
# tableI1 = []
# while i < 25:
...
#         tableI1.at[i, "raw_cas"] = str(tableI1.loc[i, "raw_cas"]).strip('/')
#         row = {'raw_chem_name': [str(tableI1.loc[i,"raw_chem_name"])], 'raw_cas': [str(tableI1.loc[i+1, "raw_cas"])]}
#         rowDF = pd.DataFrame(data=row)
#         tableI1.drop([i+1],axis=0,inplace=True)
#         tableI1.reset_index(drop=True,inplace=True)
#         lister = [rowDF,tableI1]
#         tableI1 = pd.concat(lister,ignore_index=True)
#     elif str(tableI1.at[i, "raw_cas"]).endswith(';'):
#         if ';' in str(tableI1.loc[i+1, "raw_cas"]):
#             tableI1.at[i, "raw_cas"] = str(tableI1.loc[i, "raw_cas"]).strip().strip(';')
#             continue
#         else:
#             if ';' in str(tableI1.loc[i, 'raw_cas']):
#                 tableI1.at[i,"raw_chem_name"] = str(tableI1.loc[i-1,"raw_chem_name"])
#                 tableI1.at[i+1,"raw_chem_name"] = str(tableI1.loc[i-1,"raw_chem_name"])
#                 tableI1.at[i, "raw_cas"] = str(tableI1.loc[i, "raw_cas"]).strip().strip(';')
#                 tableI1.at[i+1, "raw_cas"] = str(tableI1.loc[i+1, "raw_cas"]).strip().strip(';')       
...
#         tableI1.at[i, "raw_cas"] = str(tableI1.loc[i, "raw_cas"]).strip().strip('or')
#         row = {'raw_chem_name': [str(tableI1.loc[i,"raw_chem_name"])], 'raw_cas': [str(tableI1.loc[i+1, "raw_cas"]).strip()]}
#         rowDF = pd.DataFrame(data=row)
#         tableI1.drop([i+1],axis=0,inplace=True)
#         tableI1.reset_index(drop=True,inplace=True)
#         lister = [rowDF,tableI1]
...
#         tableI1.at[i, "raw_cas"] = str(tableI1.loc[i, "raw_cas"]).strip().strip('or')
#         row = {'raw_chem_name': [str(tableI1.loc[i-1,"raw_chem_name"])], 'raw_cas': [str(tableI1.loc[i, "raw_cas"]).strip()]}
#         rowDF = pd.DataFrame(data=row)
