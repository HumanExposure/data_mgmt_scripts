...
file = r'C:\Users\CLUTZ01\OneDrive - Environmental Protection Agency (EPA)\Profile\Documents\Extraction Scripts\DCPS_siloxanes\report.pdf'
...
# %% Table 2
table_15_raw = read_pdf(file, pages = '25', stream = True, area=[[150,70,450,448]]) #pandas_options={'header': None})

# %%% extraction and clean up
table_15=table_15_raw[0]

   
table_15.fillna(method='ffill', inplace=True)
table_15 = table_15.groupby(['component'], as_index=False)['raw_chem_name'].apply(' '.join)
table_15 = table_15[table_15["raw_chem_name"].str.contains("eclaration") == False]




table_15 = table_15.iloc[:,0]
table_15 = pd.DataFrame({'raw_chem_name': table_15.values})
table_15.dropna(how = 'all', inplace = True)
...
for j in range(0, len(table_15)):
    table_15["raw_chem_name"].iloc[j]=str(table_15["raw_chem_name"].iloc[j]).strip().lower().replace(".","").replace("α","alpha").replace('TM', '')
...
table_15["data_document_id"]="1669866"
table_15["data_document_filename"]="survey_risk_assessment_of_developers_in_thermal_paper_a.pdf"
table_15["doc_date"]="December 2019"
table_15["raw_cas"]=""
table_15["component"]=""
...
table_15.to_csv("thermal_papers_table_15.csv", columns=["data_document_id","data_document_filename","doc_date","raw_category","raw_cas","raw_chem_name","report_funcuse","cat_code","description_cpcat","cpcat_code","component","cpcat_sourcetype"], index=False)
