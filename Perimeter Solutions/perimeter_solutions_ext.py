...
# %% This is just a separate chunk to test an individual txt file


ifile = open(fileList[1])
p_text = ifile.read()
cleaned = cleanLine(p_text)
...
def extractData(fileList):
    ...
    for file in fileList:
        ...
        template = csv.reader(open(r'C:/Users/CLUTZ01/OneDrive - Environmental Protection Agency (EPA)/Profile/Documents/Extraction Scripts/Perimeter Solutions/perimeter_solutions_documents_20230118.csv')) #Get factotum document ids
        for row in template:
        ...
        cat = cleaned.split('use of the substance/mixture :')[-1].split('1.3.')[0]#.split('safety data sheet')[0].replace('\n',' ').strip(': ') #Get raw category
        ...
        for line in lines: #Extract ingredient section
            if inIngredients == True:
                if 'section 4' in line or 'CAS number' in line or 'exact concentration' in line or 'trade secret' in line: #Out of ingredients section
                    inIngredients = False
                    continue
...
# %%    Dont use this it is just for reference
...
# =============================================================================
#     execfile = "pdftotext.exe"
#     execpath = r"C:\Users\CLUTZ01\xpdf-tools-win-4.04\bin64\pdftotext.exe" #Path to execfile
#     for file in pdfs:
#         cmd = os.path.join(execpath,execfile)
#         cmd = " ".join([cmd,"-nopgbrk","-table", "-enc UTF-8",pdf])
#         os.system(cmd)
