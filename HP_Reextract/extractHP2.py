...
def extractData(fileList):
    ...
    for file in fileList:
        ...
        ID = ''
        template = csv.reader(open('Factotum_Hewlett-Packard_1_unextracted_documents_20240123 (1).csv'))
        for row in template:
        ...
        if cleaned == '':
            print('blank: ',file)
        if 'safety data sheet' not in cleaned or 'hewlett-packard' not in cleaned:
            continue
        # for line in ifile:
        ...
        for line in ifile:
            ...
            if inIngredients == True:
                ...
                if 'composition comments' in cline or 'first-aid measures' in cline:
                    inIngredients = False
                    continue
                if 'material name' in cline or 'issue date' in cline or 'chemical name' in cline or 'hazardous components' in cline: 
                    continue
    ...
    df.to_csv('HP Extracted Text 2.csv',index=False, header=True)
