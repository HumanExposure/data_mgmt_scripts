...
def extractData(fileList):
    ...
    for file in fileList:
        ...
        docRecords = csv.reader(open('sds_documents_20200902.csv')) 
        ...
        for line in ifile:
            ...
            elif 'product generic name' in cline and prodname == '': #finds product name in line
                prodname = cline.split('  ')[-1].replace('product generic name', '') # extract product name ,removes label
            ...
            if 'no data available' in cline and 'revision date' in cline and date == '': 
                date = cline.split(' ')[7].strip() # extract date
                rev  = cline.split(' ')[-1].strip()  #extract version (last item)
                #print(len(cline),cline, file)
            elif 'revision date' in cline and 'issuing date' in cline and date == '': # 3 date elements
                date = cline.split(' ')[5].strip() # extract date
                rev  = cline.split(' ')[-1].strip()  #extract version (last item)
                #print(len(cline),cline, file)
            elif 'supersedes date' in cline and'revision date' in cline and date == '': # 3 date elements
                date = cline.split(':')[2].strip('version')  # extract date
                rev = cline.split(':')[-1].strip()  #extract version (last item)
                #print(len(cline),cline, file)
            elif 'revision date' in cline and ':' in cline and date == '':            # if using : seperator
                date = cline.split(':')[1].replace('version', '') # extract date
                rev = cline.split(':')[-1].strip()  #extract version (last item)
                #print(len(cline),cline, file)
            # elif 'revision date' in cline and date == '': # two date elements
            ...
            #Get product use
            if inUse == True:
                if 'uses advised against' in cline: #where to stop getting use data 
                    inUse = False
                else:
            ...
            if inIngredients == True:
                ...
                if 'the exact percentage' in cline or 'this material is considered hazardous' in cline : #look for keyword after table
                ...
                elif len(sline) == 4: #if the line has four elements, they are ingredient name, cas, %, and trade secret
                ...
                elif len(sline) == 3: #if the line has three elements, they are ingredient name, cas, %, and trade secret
                    chem.append(sline[0]) #ingredient name
                    conc = sline[2]         # concentration 
                    cas.append(sline[1]) # CAS number
            ...
            if 'chemical name' in cline and 'cas-no' in cline and 'weight %' in cline and 'trade secret' in cline: #look for these keywords in ingredients table
                inIngredients = True
    ...
    df.to_csv(r'Sun_SDS_Extracted_Text.csv',index=False, header=True) #create a new csv to write extracted data
    
    
def main():
    os.chdir(r'C:\Users\SHanda\OneDrive - Environmental Protection Agency (EPA)\Profile\Desktop\Factotum\Composition\Sun Products\Sun SDS') #Folder pdfs are in
