...
def cleanLine(line):
    ...
    cline = line.replace('–','-').replace('≤','<=').replace('≥','>=').replace('®','').replace('™','')
    cline = cline.lower()
    # cline = re.sub(' +', ' ', cline)
    cline = cline.strip()
...
os.chdir(r'C:/Users/alarger/OneDrive - Environmental Protection Agency (EPA)/Profile/Documents/Lincoln Electric/harris')
...
for file in fileList:
    ...
    template = csv.reader(open(r'Factotum_Harris_Products_Group_SDS_documents_20231114.csv')) #Get factotum document ids
    ...
    if ID in ['1679955','1679993','1679540']: continue #different format
    ...
    prodname = cleaned.split('product name:')[-1].split('product size')[0].split('other means of identification')[0].replace('\n',' ').replace('tm','').strip() #get product name
    ...
    for line in lines: #Extract ingredient section
        ...
        if 'the components are not hazardous' in line: continue #no chemicals 
...
df.to_csv(r'harris extracted text.csv',index=False, header=True)
