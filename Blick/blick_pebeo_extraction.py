...
def cleanLine(line):
    ...
    line=line.replace('é','e').replace('É','e').replace('²','').replace('Ï','i').replace('≤','<=').replace('≥','>=')
...
for row in template:
    if row[7].split('_')[0].lower() == 'pebeo':
        filename=row[6]
        ID = row[0]
        if ID in ['1753887']: continue
    else: continue
    ...
    cat=cat.split('1.3')[0].split('1.2.2.')[0].replace('\n',' ').strip(': |>.')
    cat = re.sub(' +', ' ', cat)
    ...
    if 'section 3' in cleaned: 
        ...
        for line in lines:
            ...
            if line.replace(' ','') in ['informationoningredients:','specificconcentrationlimits:','identificationnanoform','nanoform']: break #end of ingredients table
            ...
            if any(x in line for x in ['composition/information','identification (ec)','information on ingredients','substance for which maximum workplace','composition:','no substances fulfil the criteria','3.2. mixtures','(full text of h-phrases','infodyne','composition :','identification classification (ec) 1272/2008 note %','3.1. substances']): continue #these are unimportant lines
            if 'x %' in line: 
                conc=re.findall('[0-9,.]{1,}[ <=x%]{3,10}[0-9,.]{1,}',line)
                if len(conc)==1:
            ...
            if 'cas:' in line: 
                ...
                inchem = False
            elif 'ec:' in line or 'reach:' in line or 'index:' in line:
                inchem = False
            else:
                line=line.split('skin ')[0].split('m acute')[0].split('aquatic')[0].split('m chronic')[0].split('eye ')[0].split('repr.')[0].split('acute')[0].split('dgr')[0].split('wng')[0].split('stot ')[0].split('ghs0')[0].split('asp.')[0].split('euh')[0].split('h304')[0].split('flam.')[0].split('resp.')[0].split('press.')[0].strip()
                if line!='': 
                    if inchem==False:
                        chem.append('')
                        inchem=True
    ...
    if len(cas)<len(chem): cas.append('')
    ...
    for c in range(0,len(chem)): 
        ...
        conc=re.findall('[0-9,.]{1,5}[ <=x%]{3,10}[0-9,.]{1,7}',chem[c])
        if len(conc)==1: chem[c]=chem[c].replace(conc[0],'').strip()
...
df.to_csv(r'C:\Users\alarger\OneDrive - Environmental Protection Agency (EPA)\Profile\Documents\blick uploads\pebeo Extracted Text.csv',index=False, header=True)
