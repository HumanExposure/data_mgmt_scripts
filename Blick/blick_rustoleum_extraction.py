...
i=0
unextractedids = []
template = csv.reader(open(r'c:\Users\alarger\OneDrive - Environmental Protection Agency (EPA)\Profile\Documents\blick uploads\Factotum_Blick_(M)SDS_(2024)_unextracted_documents_20250522.csv'))
for row in template:
...
for row in template:
    ...
    date=date.split('product identifier')[0].split('supercedes')[0].split('\n')[0]
    ...
    prodname = prodname.split('product identifier')[0].split('product number')[0]
    prodname = prodname.replace('revision date:','').replace(date,'')
    ...
    elif 'product use/class' in cleaned: 
        cat = cleaned.split('product use/class')[1]
    else: print('raw cat',ID,filename)
    cat=clean(cat).split('supplier')[0]
    cat=cat.replace('\n',' ').strip(':*_ .,')
    cat = re.sub(' +', ' ', cat)
    ...
    if 'information on ingredients' in cleaned:
        sec3='\n'.join(cleaned.split('information on ingredients')[1:]).split('4. first')[0]
    ...
    for line in lines:
        if betweenPages == True:
            ...
            if 'msds for' in line:
                betweenPages=False
            continue
        if 'page' in line and 'item numbers' in line:
            betweenPages=True
            continue
        
        if any(x in line for x in ['hazardous substances','chemical name','date printed','msds for']): 
            continue #skip these lines
        
        if line in ['range','ary']:
            continue #skip these lines
        
        if line == 'less than twa stel ceiling':
            lessthan=True
            continue #composition numbers are less than 
        
        line=line.split('ghs')[0]
        ...
        elif 'propriet' in line: 
            cas.append('')
            chem.append(line.split('propriet')[0].strip())
            centC.append(line.split('propriet')[1].strip())
        elif line in ['ether','paraffinic'] and len(chem)>0:
            chem[-1]=chem[-1]+' '+line
        else:
            print(ID,line)
    ...
    for c in range(0,len(chem)): 
        ...
        minC.append('')
        maxC.append('')
        centC[c]=centC[c].split('not available')[0].split('no information')[0].split('n.e.')[0].split('10 ppm')[0].split('100 ppm')[0].split('20 ppm')[0]
        if lessthan == True:
            centC[c]='0-'+centC[c]
...
df.to_csv(r'C:\Users\alarger\OneDrive - Environmental Protection Agency (EPA)\Profile\Documents\blick uploads\rustoleum extracted text.csv',index=False, header=True)
