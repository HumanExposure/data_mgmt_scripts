...
for row in template:
    if (clean(row[7].split('_')[0].replace(' ','').lower()) in ["1-shot"]):
        filename=row[6]
        ID = row[0]
    else: continue
    ...
    rev = rev.split('section')[0]
    rev = rev.replace('\n',' ').strip(':., ')
    rev = re.sub(' +', ' ', rev)
    
 
    #Raw category
    if 'use of the substance/' in cleaned: 
        cat = cleaned.split('use of the substance/')[1]
    else: print('raw cat',ID,filename)
    cat=clean(cat).split('uses advised')[0].split('mixture')[0]
    cat=cat.replace('\n',' ').strip(':*_ .,')
    cat = re.sub(' +', ' ', cat)
    ...
    if 'section 3. composition/information on ingredients' in cleaned:
        sec3='\n'.join(cleaned.split('section 3. composition/information on ingredients')[1:]).split('section 4')[0]
    ...
    for line in lines:
        ...
        if any(x in line for x in ['substance/mixture','product name','ingredient name','product code','codes represent substances','any concentration shown','no additional ingredients','concentrations applicable','in this section','occupational exposure limits']): 
            continue #skip these lines

        casrns = findcas(line)
        if len(casrns)==1:
            cas.append(casrns[0])
            line=line.split(casrns[0])[0].strip()
            ...
            for word in words:
                if word[0] in '<>':
                    ...
                    chemname=(chemname+' '+word).strip()
            
            chem.append(chemname)
            centC.append(line.replace(chemname,'').strip())
            
  
        else:
            chem[-1]=chem[-1]+' '+line.replace('931-312-3)','')
...
df.to_csv(r'C:\Users\alarger\OneDrive - Environmental Protection Agency (EPA)\Profile\Documents\blick uploads\1 shot extracted text.csv',index=False, header=True)
