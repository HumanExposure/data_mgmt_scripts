...
i=0
unextractedids = []
template = csv.reader(open(r'c:\Users\alarger\OneDrive - Environmental Protection Agency (EPA)\Profile\Documents\blick uploads\Factotum_Blick_(M)SDS_(2024)_unextracted_documents_20250515.csv'))
for row in template:
...
for row in template:
    if (clean(row[7].split('_')[0].replace(' ','').lower()) in ["laguna",'skut']):
        filename=row[6]
        ID = row[0]
    else: continue
    ...
    prodname = prodname.split('common names')[0]
    prodname = clean(prodname).replace('\n',' ').strip(':. ')
    prodname = re.sub(' +', ' ', prodname)
    
    #Date
    if 'current revision date is' in cleaned:
        date = cleaned.split('current revision date is')[1]
    else: 
        print('date',ID,filename)
    date=date.split('information')[0]
    ...
    cat=clean(cat).split('restrictions')[0]
    cat=cat.replace('non-exhaustive list','')
    cat=cat.replace('\n',' ').strip(':*_ .,')
    cat = re.sub(' +', ' ', cat)
    ...
    for line in lines:
        ...
        if any(x in line for x in ['substance/mixture','substances/mixtures','approx %']): 
            continue #skip these lines
        
        casrns = findcas(line)
        if len(casrns)==0 and '208-169-4' in line:
            casrns.append('208-169-4')
        if len(casrns)==1:
            cas.append(casrns[0])
            ...
            cas.append('')
            chem.append('dust')
            centC.append(line.split('dust')[-1].strip())
        else:
            print(ID,line)
...
df.to_csv(r'C:\Users\alarger\OneDrive - Environmental Protection Agency (EPA)\Profile\Documents\blick uploads\laguna extracted text.csv',index=False, header=True)
