# -*- coding: utf-8 -*-
"""
Created on Wed Apr 27 17:06:52 2022
...
file = 'skin_deep_babies__moms_products_raw_extracted_records (2).csv' #Name of raw extracted records file
newName = file.replace('_raw_extracted_records','_cleaned')

template = csv.reader(open(file))
for row in template:
    # row[5] = row[5].replace(',','.')
    # row[5] = row[5].replace('less than','<')
    ...
    if row[7] == 'percent':
        ...
        if row[4]+row[5]+row[6]+row[7] != '': print(row) #Print rows not being cleaned
