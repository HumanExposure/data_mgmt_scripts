...
file = 'Factotum_Hewlett-Packard_MSDS_(2017)_raw_extracted_records_20240801 (1).csv' #Name of raw extracted records file
...
for row in template:
    ...
    row[4]=row[4].replace('≥','>=')
    row[6]=row[6].replace('≤','<=')
    row[5] = row[5].replace(',','.').replace('≥','>=').replace('≤','<=').replace('~','-').replace('–','-')
    if row[4] == row[6] and row[5] == '': #Same number in min and max, move concentration to central
        row[5] = row[4]
        row[4] = ''
        row[6] = ''
    if row[7] == 'percent':
        # if '~' in row[5]: print(row)
        if row[5]=='remainder': continue
