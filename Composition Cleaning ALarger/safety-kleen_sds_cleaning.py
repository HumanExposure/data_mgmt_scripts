# -*- coding: utf-8 -*-
"""
Created on Wed Jun 24 10:12:54 2020
...
file = 'safety-kleen_sds_raw_extracted_records.csv' #Name of raw extracted records file
...
for row in template:
    if row[7] == 'percent':
        if all(n in '1234567890. ' for n in row[5]) and row[4] == '' and row[5] != '' and row[6] == '': 
            ...
            idList.append(row[1])
            lowerList.append((pd.to_numeric(row[4].strip('> '))/100).round(10))
            centList.append('')
            upperList.append((pd.to_numeric(row[6].strip('< '))/100).round(10))
