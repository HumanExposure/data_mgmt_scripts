# -*- coding: utf-8 -*-
"""
Created on Mon Apr  4 18:27:30 2022
...
file = 'unilever_1_-_msds_usa_raw_extracted_records.csv' #Name of raw extracted records file
newName = file.replace('_raw_extracted_records','_cleaned')

template = csv.reader(open(file))
for row in template:
    if row[4] == row[6] and row[5] == '':
        row[5] = row[4]
        row[4] = ''
        row[6] = ''
    if row[7] == 'percent':
    ...
    elif row[7] == 'weight fraction':
        if all(n in '1234567890., ' for n in row[4]+row[6]) and row[4] != '' and row[5] == '' and row[6] != '':
            idList.append(row[1])
            lowerList.append((pd.to_numeric(row[4])))
            centList.append('')
            upperList.append((pd.to_numeric(row[6])))
            if any(n > 1 or n < 0 for n in [lowerList[-1],upperList[-1]]) or lowerList[-1] >= upperList[-1]: #Check if wf makes sense
                print('concentration out of range:',row)
                del idList[-1]
                del lowerList[-1]
        ...
        elif all(n in '1234567890., ' for n in row[5]) and row[4] == '' and row[5] != '' and row[6] == '':
            idList.append(row[1])
            lowerList.append('')
            centList.append(pd.to_numeric(row[5]))
            upperList.append('')
            if centList[-1] > 1 or centList[-1] <= 0: #Check if wf makes sense
                print('concentration out of range:',row)
                del idList[-1]
