# -*- coding: utf-8 -*-
"""
Created on Wed Jun 24 13:27:05 2020
...
file = 'tarkett_material_health_statements_raw_extracted_records.csv' #Name of raw extracted records file
...
for row in template:
    if row[7] == 'percent':
        if all(n in '1234567890., ' for n in row[5]) and row[4] == '' and row[5] != '' and row[6] == '': 
            idList.append(row[1])
            lowerList.append('')
            centList.append((pd.to_numeric(row[5].replace(',','.'))/100).round(10))
            upperList.append('')
            ...
            idList.append(row[1])
            lowerList.append((pd.to_numeric(row[4].replace(',','.'))/100).round(10))
            centList.append('')
            upperList.append((pd.to_numeric(row[6].replace(',','.'))/100).round(10))
