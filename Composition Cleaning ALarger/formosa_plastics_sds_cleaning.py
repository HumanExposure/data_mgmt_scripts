...
file = 'Factotum_Formosa_Plastics_SDS_raw_extracted_records_20260331.csv' #Name of raw extracted records file
newName = file.replace('_raw_extracted_records','_cleaned')

template = csv.reader(open(file,encoding='utf8'))
for row in template:
    row[5] = row[5].replace(',','.')
    if row[5] == 'impurityresidual': continue
    row[5]=row[5].replace('alt','').strip()
    row[4]=row[4].replace('alt','').strip()
    row[6]=row[6].replace('alt','').strip()
    ...
    if row[7] == 'percent':
        ...
        elif row[4]!='' or row[5]!='' or row[6]!='': print(row) #Print rows not being cleaned
    elif row[4]!='' or row[5]!='' or row[6]!='': print(row) #Print rows not being cleaned
