...
file = 'Factotum_Eastman_Products_(auto)_raw_extracted_records_20231109.csv' #Name of raw extracted records file
...
for row in template:
    ...
    row[5] = row[5].replace('≥','>=').replace('≤','<=').replace('~','').replace(',','.')
    row[6] = row[6].replace('≥','>=').replace('≤','<=').replace('~','').replace(',','.')
