...
file = 'Factotum_3M_Occupational_Health_and_Safety_(2019)_raw_extracted_records_20240624.csv' #Name of raw extracted records file
...
for row in template:
    ...
    if row[7] == 'percent':
        ...
        #Split up ranges
        row[5] = row[5].replace('approximately','')
        row[5] = row[5].strip()
        ...
        if row[5].count('-') == 1:
            ...
            row[5] = ''
        elif row[5][0] == '<' and row[4]=='' and row[6]=='':
            row[4] = '0'
            row[6] = row[5].strip('< ')
            row[5] = ''
        elif row[5][0] == '>' and row[4]=='' and row[6]=='':
