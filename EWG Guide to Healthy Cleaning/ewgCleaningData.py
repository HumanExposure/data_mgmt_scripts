...
Reads in csv of extracted data, separates and cleans ingredient list, and formats data in a template that can be uploaded to factotum
"""

import os, csv, re, string
import pandas as pd

os.chdir(r'L:\Lab\HEM\ALarger\EWG Guide to Healthy Cleaning')    
...
extracted = csv.reader(open(r'ewg healthy cleaning products.csv','r'))
i = 0
k = 0
for row in extracted:
    ...
    template = csv.reader(open(r'ewg_guide_to_healthy_cleaning_documents_20190829.csv')) #"Document records" file from Factotum
    docID = ''
    for row in template:
    ...
    if i%1000 == 0: #create a new csv every 1000 products so that it can be loaded into factotum
        ...
        df.to_csv('EWG Cleaning Upload' + str(k) + '.csv',index=False, header=True)
        idList = [] 
        filenameList = [] 
        prodList = [] 
        dateList = [] 
