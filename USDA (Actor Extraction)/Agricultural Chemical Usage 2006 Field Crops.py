# -*- coding: utf-8 -*-
"""
Created on Tue May 21 16:51:36 2019
...
os.chdir(r'L:\\Lab\\HEM\\ALarger\\Actor Automated Extraction\\USDA\\Annual Field Crops Summaries 2006')    
execfile = "pdftotext.exe"
execpath = 'C:\\Users\\alarger\\xpdf-tools-win-4.00\\bin64\\' #Path to execfile
cmd = os.path.join(execpath,execfile)
cmd = " ".join([cmd,"-nopgbrk","-layout",'"2006 Fungicides.pdf"'])
os.system(cmd) #Convert pdf to text
ifile = open(r'L:\Lab\HEM\ALarger\Actor Automated Extraction\USDA\Annual Field Crops Summaries 2006\2006 Fungicides.txt')
...
for line in ifile:
    ...
    if inClasses == True:
        ...
        try:
            if 'H' in cline[0]:
                ...
                templateName.append('2006 Herbicides.pdf')
            if 'I' in cline[0]:
                chemName.append(cline[1])
                templateName.append('2006 Insecticides.pdf')
            if 'F' in cline[0]:
                chemName.append(cline[1])
                templateName.append('2006 Fungicides.pdf')
            if 'O' in cline[0]:
                chemName.append(cline[1])
                templateName.append('2006 Other Chemicals.pdf')
        except: pass
        continue
    cline = cline.strip()
    if cline == '': continue
    ...
    if inChems == True: 
        ...
        templateName.append('2006 ' + fruit + '.pdf')
        chemName[-1] = chemName[-1].rstrip(' 1').rstrip(' 2').rstrip(' 3').rstrip(' 4')

templateName.extend(['2006 Fertilizers.pdf']*4)
chemName.extend(['Nitrogen','Phosphate','Potash','Sulfur'])

for t in templateName:
    template = csv.reader(open(r'L:\Lab\HEM\ALarger\Actor Automated Extraction\USDA\Annual Field Crops Summaries 2006\usda-agricultural-chemical-usage-2006-field-crops_20190521.csv'))
    for row in template:
        if row[3] == t:
            prodID.append(row[0])
            break
...
msdsDate = ['May 2007']*nIngredients
recUse = ['']*nIngredients
...
df.to_csv(r'L:\Lab\HEM\ALarger\Actor Automated Extraction\USDA\Annual Field Crops Summaries 2006\USDA Field Crops 2006.csv',index=False, header=True)