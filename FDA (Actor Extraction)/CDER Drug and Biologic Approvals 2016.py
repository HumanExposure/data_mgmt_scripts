...
# os.chdir(r"C:\Users\CLUTZ01\OneDrive - Environmental Protection Agency (EPA)\Profile\Documents\Projects\Extraction Scripts\FDA\CDER Drug and Biologic Approvals\2016")
...
file = r"C:\Users\CLUTZ01\OneDrive - Environmental Protection Agency (EPA)\Profile\Documents\Projects\Extraction Scripts\FDA\CDER Drug and Biologic Approvals\2016\2016.pdf"
...
for i,table in enumerate(tables):
    ...
    if i <= 4 : #New Drug Approvals
        ...
        templateName.extend(['cder_drug_and_biological_approvals_2016_nda.pdf']*len(df))
        components.extend(['']*len(df))
    else: #Table 7
        chemName.extend(df.iloc[:,2])
        prodID.extend(['1690584']*len(df))
        templateName.extend(['cder_drug_and_biological_approvals_2016_bla.pdf']*len(df))
        components.extend(['']*len(df))
...
os.chdir(r"C:\Users\CLUTZ01\OneDrive - Environmental Protection Agency (EPA)\Profile\Documents\Projects\Extraction Scripts\FDA\CDER Drug and Biologic Approvals\2016")
df.to_csv(r'2016_cder_drug_approvals_ext.csv',index=False, header=True, date_format=None)
