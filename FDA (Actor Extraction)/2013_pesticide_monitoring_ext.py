...
# os.chdir(r"C:\Users\CLUTZ01\OneDrive - Environmental Protection Agency (EPA)\Profile\Documents\Projects\Extraction Scripts\FDA\Pesticide Residue Monitoring\2013")
...
file = r"C:\Users\CLUTZ01\OneDrive - Environmental Protection Agency (EPA)\Profile\Documents\Projects\Extraction Scripts\FDA\Pesticide Residue Monitoring\2013\2013.pdf"
tables = camelot.read_pdf(file,pages='22-33', flavor='lattice')
...
for i,table in enumerate(tables):
    df = tables[i].df
    if i <= 3: #Table 3 (need to add zoxamide in manually)
        chemName.extend(df.iloc[:,0])
        chemName.extend(df.iloc[:,1])
        chemName.extend(df.iloc[:,2])
        prodID.extend(['1690504']*len(df)*3)
        templateName.extend(['2013_pesticide_monitoring_report_table_3.pdf']*len(df)*3)
        components.extend(['']*len(df)*3)
    elif i >= 5 and i<=6: #Table 5
        chemName.extend(df.iloc[:,0])
        prodID.extend(['1690505']*len(df))
        templateName.extend(['2013_pesticide_monitoring_report_table_5.pdf']*len(df))
        components.extend(['']*len(df))
    elif i == 8: #Table 6c
        chemName.extend(df.iloc[:,0])
        prodID.extend(['1690508']*len(df))
        templateName.extend(['2013_pesticide_monitoring_report_table_6c.pdf']*len(df))
        components.extend(['']*len(df))

    elif i == 9: #Table 6d
        chemName.extend(df.iloc[:,2])
        prodID.extend(['1690506']*len(df))
        templateName.extend(['2013_pesticide_monitoring_report_table_6d.pdf']*len(df))
        ...
        templateName.extend(['2013_pesticide_monitoring_report_table_7.pdf']*len(df))
        components.extend(['']*len(df))
...
os.chdir(r"C:\Users\CLUTZ01\OneDrive - Environmental Protection Agency (EPA)\Profile\Documents\Projects\Extraction Scripts\FDA\Pesticide Residue Monitoring\2013")
df.to_csv(r'2013_pesticide_monitoring_ext.csv',index=False, header=True, date_format=None)
