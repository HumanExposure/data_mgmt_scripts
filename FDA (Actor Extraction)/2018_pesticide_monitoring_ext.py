...
# os.chdir(r"C:\Users\CLUTZ01\OneDrive - Environmental Protection Agency (EPA)\Profile\Documents\Projects\Extraction Scripts\FDA\Pesticide Residue Monitoring\2018")
...
file = r"C:\Users\CLUTZ01\OneDrive - Environmental Protection Agency (EPA)\Profile\Documents\Projects\Extraction Scripts\FDA\Pesticide Residue Monitoring\2018\2018.pdf"
tables = camelot.read_pdf(file,pages='23-25,28', flavor='lattice')
...
for i,table in enumerate(tables):
    ...
    if i <= 2: #Table 3
        ...
        prodID.extend(['1690546']*len(df)*3)
        templateName.extend(['2018_pesticide_monitoring_report_table_3.pdf']*len(df)*3)
        components.extend(['']*len(df)*3)
    elif i == 3: #Table 5
        chemName.extend(df.iloc[:,0])
        prodID.extend(['1690547']*len(df))
        templateName.extend(['2018_pesticide_monitoring_report_table_5.pdf']*len(df))
...
while j > 0: #go through list backwards, so it doesnt mess up the index if a row is deleted
    ...
    if prodID[j] != '1690546': chemName[j] = chemName[j].replace('ǂ', '').replace('*', '').rstrip('2').rstrip('3').rstrip('4').rstrip('5')
...
os.chdir(r"C:\Users\CLUTZ01\OneDrive - Environmental Protection Agency (EPA)\Profile\Documents\Projects\Extraction Scripts\FDA\Pesticide Residue Monitoring\2018")
df.to_csv(r'2018_pesticide_monitoring_ext.csv',index=False, header=True, date_format=None)
