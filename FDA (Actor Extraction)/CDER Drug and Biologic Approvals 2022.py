...
# os.chdir(r"C:\Users\CLUTZ01\OneDrive - Environmental Protection Agency (EPA)\Profile\Documents\Projects\Extraction Scripts\FDA\CDER Drug and Biologic Approvals\2022")
...
file = r"C:\Users\CLUTZ01\OneDrive - Environmental Protection Agency (EPA)\Profile\Documents\Projects\Extraction Scripts\FDA\CDER Drug and Biologic Approvals\2022\2022.pdf"
...
for i,table in enumerate(tables):
    ...
    if i < 4 : #New Drug Approvals
        ...
        prodID.extend(['1690603']*len(df))
        templateName.extend(['cder_drug_and_biological_approvals_2022_nda.pdf']*len(df))
        components.extend(['']*len(df))
    else: #Table 7
        chemName.extend(df.iloc[:,2])
        prodID.extend(['1690604']*len(df))
        templateName.extend(['cder_drug_and_biological_approvals_2022_bla.pdf']*len(df))
        components.extend(['']*len(df))
...
while j > 0: #go through list backwards, so it doesnt mess up the index if a row is deleted
    ...
    if prodID[j] != '1690603': chemName[j] = chemName[j].replace('ǂ', '').replace('*', '').rstrip('2').rstrip('3').rstrip('4').rstrip('5')
...
os.chdir(r"C:\Users\CLUTZ01\OneDrive - Environmental Protection Agency (EPA)\Profile\Documents\Projects\Extraction Scripts\FDA\CDER Drug and Biologic Approvals\2022")
df_drop.to_csv(r'2022_cder_drug_approvals_ext.csv',index=False, header=True, date_format=None)
