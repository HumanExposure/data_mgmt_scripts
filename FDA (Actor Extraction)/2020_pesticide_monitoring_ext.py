...
# os.chdir(r"C:\Users\CLUTZ01\OneDrive - Environmental Protection Agency (EPA)\Profile\Documents\Projects\Extraction Scripts\FDA\Pesticide Residue Monitoring\2020")
...
file = r"C:\Users\CLUTZ01\OneDrive - Environmental Protection Agency (EPA)\Profile\Documents\Projects\Extraction Scripts\FDA\Pesticide Residue Monitoring\2020\2020.pdf"
tables = camelot.read_pdf(file,pages='23-24,29', flavor='lattice')
...
for i,table in enumerate(tables):
    ...
    if i <= 1: #Table 3
        ...
        prodID.extend(['1690555']*len(df)*3)
        templateName.extend(['2020_pesticide_monitoring_report_table_3.pdf']*len(df)*3)
        components.extend(['']*len(df)*3)
    elif i == 2: #Table 7
        chemName.extend(df.iloc[:,0])
        chemName.extend(df.iloc[:,1])
        chemName.extend(df.iloc[:,2])
        prodID.extend(['1690556']*len(df)*3)
        templateName.extend(['2020_pesticide_monitoring_report_table_7.pdf']*len(df)*3)
        components.extend(['']*len(df)*3)
...
while j > 0: #go through list backwards, so it doesnt mess up the index if a row is deleted
    ...
    if prodID[j] != '1690555': chemName[j] = chemName[j].replace('ǂ', '').replace('*', '').rstrip('2').rstrip('3').rstrip('4').rstrip('5')
...
os.chdir(r"C:\Users\CLUTZ01\OneDrive - Environmental Protection Agency (EPA)\Profile\Documents\Projects\Extraction Scripts\FDA\Pesticide Residue Monitoring\2020")
df.to_csv(r'2020_pesticide_monitoring_ext.csv',index=False, header=True, date_format=None)
