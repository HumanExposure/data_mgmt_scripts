...
# os.chdir(r"C:\Users\CLUTZ01\OneDrive - Environmental Protection Agency (EPA)\Profile\Documents\Projects\Extraction Scripts\FDA\CDER Drug and Biologic Approvals\2012")
...
file = r"C:\Users\CLUTZ01\OneDrive - Environmental Protection Agency (EPA)\Profile\Documents\Projects\Extraction Scripts\FDA\CDER Drug and Biologic Approvals\2012\2012.pdf"
...
for i,table in enumerate(tables):
    ...
    if i <= 1 : #New Drug Approvals
        ...
        prodID.extend(['1690583']*len(df))
        templateName.extend(['cder_drug_and_biological_approvals_2012_nda.pdf']*len(df))
        components.extend(['']*len(df))
    else: #Table 7
        chemName.extend(df.iloc[:,2])
        prodID.extend(['1690584']*len(df))
        templateName.extend(['cder_drug_and_biological_approvals_2012_bla.pdf']*len(df))
        components.extend(['']*len(df))
...
while j > 0: #go through list backwards, so it doesnt mess up the index if a row is deleted
    ...
    if prodID[j] != '1690583': chemName[j] = chemName[j].replace('ǂ', '').replace('*', '').rstrip('2').rstrip('3').rstrip('4').rstrip('5')
...
os.chdir(r"C:\Users\CLUTZ01\OneDrive - Environmental Protection Agency (EPA)\Profile\Documents\Projects\Extraction Scripts\FDA\CDER Drug and Biologic Approvals\2013")
df.to_csv(r'2012_cder_drug_approvals_ext.csv',index=False, header=True, date_format=None)
