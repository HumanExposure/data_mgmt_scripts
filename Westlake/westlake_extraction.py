...
def cleanLine(line):
    ...
    line=line.replace('é','e').replace('É','e').replace('À','a').replace('≤','<=').replace('≥','>=').replace('','').replace('','').replace('™','').replace('®','')#.replace('α','alpha').replace('ω','omega').replace('β','beta').replace('γ','gamma')
...
os.chdir(r'C:\Users\alarger\OneDrive - Environmental Protection Agency (EPA)\Profile\Documents\westlake') #Folder pdfs are in
pdfs = glob("*.pdf")
txts = glob("*.txt")
...
i=0
template = csv.reader(open('Factotum_Westlake_Corporation_SDS_unextracted_documents_20250822.csv')) 
for row in template:
    filename=row[1]
    ID = row[0]
    ...
    if ID in['1799752','1799751']: continue
    ...
    date=date.split('\n')[0].split('issue')[0].split('1 /')[0].split('1/')[0].strip(':. ')
    date = re.sub(' +', ' ', date)
    ...
    rev=rev.split('revision')[0].split('issue')[0].strip(':. ')
    rev = re.sub(' +', ' ', rev)
    ...
    cat = cat.split('recommended restrictions')[0]
    cat = (cat).replace('\n',' ').strip(':.- ')
    cat = re.sub(' +', ' ', cat)
    ...
    for line in lines:
        ...
        if any(x in line for x in ['common name and synonyms','revision date','material name:','the manufacturer lists no ingredients','issue date']):     
...
df.to_csv('westlake extracted text.csv',index=False, header=True)
