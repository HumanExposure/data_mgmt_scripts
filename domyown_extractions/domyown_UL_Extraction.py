...
os.chdir(r'C:\Users\alarger\OneDrive - Environmental Protection Agency (EPA)\Profile\Documents\domyown\Factotum_DoMyOwn_SDS_2024_documents_20251215') #Folder pdfs are in
pdfs = glob("*.pdf")
txts = glob("*.txt")
...
i=0
template = csv.reader(open(r'c:\Users\alarger\OneDrive - Environmental Protection Agency (EPA)\Profile\Documents\domyown\Factotum_DoMyOwn_SDS_(2024)_unextracted_documents_20260107.csv')) 
for row in template:
    filename=row[1]
    ID = row[0]
    
    
    if filename not in pdfs: continue
...
df.to_csv(r'C:\Users\alarger\OneDrive - Environmental Protection Agency (EPA)\Profile\Documents\domyown\UL domyown Extracted Text.csv',index=False, header=True)
