...
def extractData(fileList):
    ...
    for file in fileList:
        ...
        template = csv.reader(open('Factotum_Diversey_SDS_unextracted_documents_20240118.csv')) #Get Factotum ID
        for row in template:
        ...
        if 'reckitt benckiser llc' not in cleaned: continue
    ...
    df.to_csv('diversey rb sds extracted text.csv',index=False, header=True)
    
    
def main():
    os.chdir(r'C:/Users/alarger/OneDrive - Environmental Protection Agency (EPA)/Profile/Documents/Diversey') #Folder pdfs are in
    pdfs = glob("*_sds.pdf")
    txts = glob("*_sds.txt")
    unconverted = []
    for n in pdfs:
