...
#takes a dataframe and removes rows where any comp data is larger than 1 or less than 0, the lower_wf_analysis is larger thena the upper_wf_analysis,the lower_wf_analysis=upper_wf_analysis, or the lower or upper wf_analysis is
...
#removes "wt", "near" from raw_central_comp point estimate values then removes all remaining rows with alphabetic characters and extraneous characters
raw_data.loc[raw_data["raw_central_comp"].str.contains("/*w[t]*|^near"), ["raw_central_comp"]]=raw_data["raw_central_comp"].str.replace("[wtnear/()]","")
