...
raw_data=pd.read_csv("siri_cpcat_data_8_raw_extracted_records.csv", usecols=["ExtractedChemical_id","raw_min_comp","raw_central_comp","raw_max_comp","unit_type__title"], dtype=str)
...
#removes "wt", from raw_central_comp point estimate values then removes all remaining rows with alphabetic characters and extraneous characters
raw_data.loc[raw_data["raw_central_comp"].str.contains("/*w[t]*"), ["raw_central_comp"]]=raw_data["raw_central_comp"].str.replace("[wt/()]","")
...
raw_data.loc[raw_data.raw_max_comp.str.contains("wt"), ["raw_max_comp"]]=raw_data.raw_max_comp.str.strip("/wt")
...
for i in range(1,math.ceil(len(clean_data)/7500)+1):
    ...
    temp.to_csv("siri_8_cleaned_%d.csv"%i, index=False)
    start=stop
