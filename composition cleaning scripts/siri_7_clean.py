...
def min_split(df=pd.DataFrame()):
    min_keys=["^>","min$|min\.$|^min[^.]","\+$","over"]
    df.loc[(df["raw_central_comp"].str.contains("|".join(min_keys))), ["raw_min_comp"]]=df["raw_central_comp"].str.replace("[min+>over()_/=]","").str.strip(".")
...
def max_split(df=pd.DataFrame()):
    max_keys=["^<","m[a]*x","upto","under"]
    df.loc[df["raw_central_comp"].str.contains("|".join(max_keys)), ["raw_min_comp"]]="0"
    df.loc[df["raw_central_comp"].str.contains("|".join(max_keys)), ["raw_max_comp"]]=df["raw_central_comp"].str.replace("[maxuptonder<_()]","")
...
raw_data=pd.read_csv("siri_cpcat_data_7_raw_extracted_records.csv", usecols=["ExtractedChemical_id","raw_min_comp","raw_central_comp","raw_max_comp","unit_type__title"], dtype=str)
...
#when raw_central_comp is xor>x, x is placed in raw_min_comp and 100 is placed in raw_max_comp
raw_data.loc[(raw_data.raw_central_comp.str.contains("or\>")) & (raw_data.raw_central_comp.str.get(0)==raw_data.raw_central_comp.str.get(-1)), ["raw_min_comp"]]=raw_data.raw_central_comp.str.get(0)
raw_data.loc[(raw_data.raw_central_comp.str.contains("or\>")) & (raw_data.raw_central_comp.str.get(0)==raw_data.raw_central_comp.str.get(-1)), ["raw_max_comp"]]="100"
raw_data.loc[(raw_data.raw_central_comp.str.contains("or\>")) & (raw_data.raw_central_comp.str.get(0)==raw_data.raw_central_comp.str.get(-1)), ["raw_central_comp"]]=""

#splits raw central comp of format x to y into min and max fields
raw_data.loc[raw_data.raw_central_comp.str.contains("\d+to\d+"), ["raw_min_comp"]]=raw_data.raw_central_comp.str.split("to",expand=True)[0]
raw_data.loc[raw_data.raw_central_comp.str.contains("\d+to\d+"), ["raw_max_comp"]]=raw_data.raw_central_comp.str.split("to",expand=True)[1]
raw_data.loc[raw_data.raw_central_comp.str.contains("\d+to\d+"), ["raw_central_comp"]]=""


#removes "wt", "apprx", etc. from raw_central_comp point estimate values then removes all remaining rows wiht alphabetic characters and extraneous characters
raw_data.loc[raw_data["raw_central_comp"].str.contains("/*w[t]*|x$"), ["raw_central_comp"]]=raw_data["raw_central_comp"].str.replace("[aprxwt/()]","")
...
#cases where raw_min_comp is is >x and raw_max_comp is <y. x is placed in raw_min_comp, y is placed in raw_max_comp, and empty string is plced in raw_central_comp
raw_data.loc[(raw_data.raw_min_comp.str.startswith(">")) & (raw_data.raw_max_comp.str.startswith("<")), ["raw_central_comp"]]=False
raw_data.loc[raw_data.raw_central_comp==False, ["raw_min_comp"]]=raw_data.raw_min_comp.str.strip(">")
raw_data.loc[raw_data.raw_central_comp==False, ["raw_max_comp"]]=raw_data.raw_max_comp.str.strip("<")
raw_data.loc[raw_data.raw_central_comp==False, ["raw_central_comp"]]=""

#There are cases where on the original document the comp data has the format >x-y. x was kept in the raw_min_comp field and10 0 was placed in the raw_max_comp field.
raw_data.loc[raw_data.raw_min_comp.str.startswith(">"), ["raw_max_comp"]]="100"
raw_data.loc[raw_data.raw_min_comp.str.startswith(">"), ["raw_min_comp"]]=raw_data.raw_min_comp.str.strip(">")
...
for i in range(1,math.ceil(len(clean_data)/7500)+1):
    ...
    temp.to_csv("siri_7_cleaned_%d.csv"%i, index=False)
    start=stop
