...
def min_split(df=pd.DataFrame()):
    ...
    df.loc[(df["raw_central_comp"].str.contains("|".join(min_keys))) | (df["raw_central_comp"].str.endswith("+")), ["raw_min_comp"]]=df["raw_central_comp"].str.replace("[minover\+><]","")
...
def max_split(df=pd.DataFrame()):
    ...
    df.loc[df["raw_central_comp"].str.contains("|".join(max_keys)), ["raw_max_comp"]]=df["raw_central_comp"].str.replace("[maxiupto/><]","")
...
raw_data=pd.read_csv("siri_cpcat_data_3_raw_extracted_records.csv", usecols=["ExtractedChemical_id","raw_min_comp","raw_central_comp","raw_max_comp","unit_type__title"])
...
#drops all rows in raw_min_comp that contain alphabetic characters
raw_data=raw_data.loc[~raw_data.raw_min_comp.str.contains("[a-zA-Z]")]

#There are cases where on the original documen the comp data has the format <x-y. y was kept in the raw_max_comp field and 0 was placed in the raw_min_comp field.
...
#removes "w" and"wt" from raw_central_comp point estimate values
...
for i in range(1,math.ceil(len(clean_data)/7500)+1):
    ...
    temp.to_csv("siri_3_cleaned_%d.csv"%i, index=False)
    start=stop
