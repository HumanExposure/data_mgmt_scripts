...
raw_data=pd.read_csv("3m_industrial_adhesives_and_tapes_raw_extracted_records.csv", usecols=["ExtractedChemical_id","raw_min_comp","raw_central_comp","raw_max_comp","unit_type__title"])
raw_data=raw_data.loc[(raw_data.unit_type__title=="percent") | (raw_data.unit_type__title=="weight fraction")]
raw_data=raw_data.loc[(raw_data.raw_central_comp.str.replace(" ","").str.contains("[A-Za-z]")==False) | (pd.isnull(raw_data.raw_central_comp))]

raw_data=raw_data.reset_index()
raw_data=raw_data[["ExtractedChemical_id","raw_min_comp","raw_central_comp","raw_max_comp","unit_type__title"]]
raw_data.loc[pd.isnull(raw_data.raw_central_comp), ["raw_central_comp"]]=""
