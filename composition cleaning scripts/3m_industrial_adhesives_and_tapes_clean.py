...
#read in data and only keep those with units of percent and errant point that is weight fraction but should be percent
raw_data=pd.read_csv("3m_industrial_adhesives_and_tapes_raw_extracted_records.csv", usecols=["ExtractedChemical_id","raw_min_comp","raw_central_comp","raw_max_comp","unit_type__title"], dtype=str)
raw_data=raw_data.loc[(raw_data.unit_type__title=="percent") | (raw_data.unit_type__title=="weight fraction")]
raw_data=raw_data.fillna("")

#clean up df
raw_data=raw_data.loc[raw_data.raw_central_comp.str.contains("not applicable", regex=False)==False]
...
raw_data.raw_central_comp=raw_data.raw_central_comp.str.replace("[a-zA-Z]","")
