...
def min_split(df=pd.DataFrame()):
    min_keys=["^>","min","\+$","<$","more"]
    df.loc[(df["raw_central_comp"].str.contains("|".join(min_keys))), ["raw_min_comp"]]=df["raw_central_comp"].str.replace("[minueor()<>+_/=]","").str.strip(".")
...
def max_split(df=pd.DataFrame()):
    bad_comps=list(df.ExtractedChemical_id.loc[df.raw_central_comp.str.contains("mm|vol")])
    ...
    df.loc[df["raw_central_comp"].str.contains("|".join(max_keys)), ["raw_max_comp"]]=df["raw_central_comp"].str.replace("^upto|[vlmaxup<_()/:]","")
...
raw_data=pd.read_csv("siri_cpcat_data_11_raw_extracted_records.csv", usecols=["ExtractedChemical_id","raw_min_comp","raw_central_comp","raw_max_comp","unit_type__title"], dtype=str)
...
#splits raw central comp of format "x to y" or "ap x to y" into min and max fields
raw_data.loc[raw_data.raw_central_comp.str.contains("\d+to\d+"), ["raw_min_comp"]]=raw_data.raw_central_comp.str.split("to",expand=True)[0].str.strip("ap")
...
raw_data=raw_data.loc[~((raw_data.raw_central_comp.str.contains("^to")) | (raw_data.raw_central_comp.str.contains("^(approx)+[.]+")))]
raw_data.loc[raw_data["raw_central_comp"].str.contains("/*w[t]*|about|approx"), ["raw_central_comp"]]=raw_data["raw_central_comp"].str.replace("[wtabourpxy/():]","")
...
raw_data=raw_data.loc[~(raw_data.raw_max_comp.str.contains("[a-zA-Z*_+()/#?]|]|\.\d\."))]
...
for i in range(1,math.ceil(len(clean_data)/7500)+1):
    ...
    temp.to_csv("siri_11_cleaned_%d.csv"%i, index=False)
    start=stop
