...
def min_split(df=pd.DataFrame()):
    min_keys=["^>","min","high"]
    df.loc[(df["raw_central_comp"].str.contains("|".join(min_keys))) | (df["raw_central_comp"].str.endswith("+")), ["raw_min_comp"]]=df["raw_central_comp"].str.replace("[minhg\+>bywt\(\)/=]","")
...
def max_split(df=pd.DataFrame()):
    ...
    df.loc[df["raw_central_comp"].str.contains("|".join(max_keys)), ["raw_max_comp"]]=df["raw_central_comp"].str.replace("[maxuptolwby<\_\(\)]","")
...
raw_data=pd.read_csv("siri_cpcat_data_4_raw_extracted_records.csv", usecols=["ExtractedChemical_id","raw_min_comp","raw_central_comp","raw_max_comp","unit_type__title"], dtype=str)
...
raw_data.loc[raw_data["raw_central_comp"].str.contains("/*w[t]*|app|\*"), ["raw_central_comp"]]=raw_data["raw_central_comp"].str.replace("[aprxwt/\(\)\*]","")
...
for i in range(1,math.ceil(len(clean_data)/7500)+1):
    ...
    temp.to_csv("siri_4_cleaned_%d.csv"%i, index=False)
    start=stop
